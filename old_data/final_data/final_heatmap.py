...
def plot_heatmaps(data, file_name, full_regex, hidden_size, analysis_type, accuracy_limits, loss_limits):
    filtered_data = data[data['Hidden Size'] == hidden_size]
    relevant_data = filtered_data[['Scale Factor', 'Sequence Length', 'Accuracy', 'Average Loss']]

    avg_accuracy = relevant_data.groupby(['Scale Factor', 'Sequence Length']).mean()['Accuracy'].unstack()
    mean_loss = relevant_data.groupby(['Scale Factor', 'Sequence Length']).mean()['Average Loss'].unstack()

    fig, axs = plt.subplots(1, 2, figsize=(18, 8))
    sns.heatmap(avg_accuracy, ax=axs[0], cmap='viridis', annot=True, vmin=accuracy_limits[0], vmax=accuracy_limits[1])
    axs[0].set_title(f'Average Accuracy for {full_regex} (Hidden Size {hidden_size})')
    ...
    axs[1].set_title(f'Mean Loss for {full_regex} (Hidden Size {hidden_size})')
    axs[1].set_xlabel('Sequence Length')
    axs[1].set_ylabel('Scale Factor')

    plt.tight_layout()

    output_dir = f'png/{analysis_type}/{full_regex}'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    plt.savefig(f'{output_dir}/heatmap_{full_regex}_hidden_{hidden_size}_{file_name}.png')
    plt.close()
...
for file_name, full_regex in file_to_regex.items():
    ...
    hidden_sizes = processed_data['Hidden Size'].unique()

    for hidden_size in hidden_sizes:
        plot_heatmaps(processed_data, file_name, full_regex, hidden_size, analysis_type, accuracy_limits, loss_limits)
