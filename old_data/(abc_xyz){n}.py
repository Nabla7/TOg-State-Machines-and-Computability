...
file_path_no_noise = '(abc_xyz){3}.csv'
file_path_noise = '(abc_xyz){6}.csv'
...
axs[0, 0].set_title('Average Accuracy - Shallow RNN for (abc|xyz){3}')
axs[0, 0].set_xlabel('Hidden Size')
axs[0, 0].set_ylabel('Sequence Length')

# Accuracy with noise
sns.heatmap(avg_accuracy_noise, ax=axs[0, 1], cmap='viridis')
axs[0, 1].set_title('Average Accuracy - Shallow RNN for (abc|xyz){6}')
...
axs[1, 0].set_title('Mean Loss - Shallow RNN for (abc|xyz){3}')
...
axs[1, 1].set_title('Mean Loss - Shallow RNN for (abc|xyz){6}')
axs[1, 1].set_xlabel('Hidden Size')
axs[1, 1].set_ylabel('Sequence Length')

plt.savefig('(abc|xyz){n}.png')
plt.tight_layout()
plt.show()
