...
file_path_no_noise = 'shortsequence_nonoise_(0_1)_0{2,4}(0_1)_1{2,4}.csv'
file_path_noise = 'shortsequence_withnoise_(0_1)_0{2,4}(0_1)_1{2,4}.csv'
...
axs[0, 0].set_title('Average Accuracy - Shallow RNN (No Noise) for (0|1)*0{2,4}(0|1)*1{2,4}')
axs[0, 0].set_xlabel('Hidden Size')
axs[0, 0].set_ylabel('Sequence Length')

# Accuracy with noise
sns.heatmap(avg_accuracy_noise, ax=axs[0, 1], cmap='viridis')
axs[0, 1].set_title('Average Accuracy - Shallow RNN (With Noise) for (0|1)*0{2,4}(0|1)*1{2,4}')
...
axs[1, 0].set_title('Mean Loss - Shallow RNN (No Noise) for (0|1)*0{2,4}(0|1)*1{2,4}')
...
axs[1, 1].set_title('Mean Loss - Shallow RNN (With Noise) for (0|1)*0{2,4}(0|1)*1{2,4}')
axs[1, 1].set_xlabel('Hidden Size')
axs[1, 1].set_ylabel('Sequence Length')

plt.savefig('(0|1)*0{2,4}(0|1)*1{2,4}_short.png')
plt.tight_layout()
plt.show()
